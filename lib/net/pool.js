...
class Pool extends EventEmitter {
  ...
  init() {
    ...
    this.chain.on('reset', async () => {
      this.forceSync();
    });

    this.chain.on('detach', async () => {
      this.resolveHeaders();
    });
    ...
    this.chain.on('bad block', (err, id) => {
      this.handleBad('block', err, id);
    });
  ...
  setLoader(peer) {
    ...
    // Set the time that the peer should stall
    // if recent headers have not been received.
    this.setHeadersTimeout(peer);

    this.emit('loader', peer);
  }

  /**
   * Set the time that the peer should timeout if recent
   * headers are not received from the peer.
   * @private
   */

  setHeadersTimeout(peer) {
    const now = Date.now();
    const best = this.chain.mostWork();
    const spacing = this.network.pow.targetSpacing;
    const count = (this.network.now() - best.time) / spacing;

    let timeout = now;
    timeout += Peer.HEADERS_TIMEOUT_BASE;
    timeout += Peer.HEADERS_TIMEOUT_PER_HEADER * count;

    peer.headersTimeout = Math.ceil(timeout);
  }
  ...
  async resync(force) {
    ...
    let locator;
    try {
      locator = await this.chain.getLocator(best.hash);
    } catch (e) {
      this.emit('error', e);
      return;
    }
  ...
  async sendSync(peer) {
    ...
    let locator;
    try {
      // Reference: Bitcoin Core commit 53b86d0de5440ffb94f6742e26dd1fd9e11952f2
      // The first time we send a locator to a new peer we start from the header
      // preceeding the best known headers. This ensures that the peer sends us
      // their tip back instead of an empty list. This way we can initialize the
      // `besthash` of this peer.
      const best = this.chain.mostWork();
      let prev = await this.chain.getPrevious(best);
      if (prev)
        prev = prev.hash;
      locator = await this.chain.getLocator(prev);
    } catch (e) {
      peer.syncing = false;
      this.emit('error', e);
      return false;
  ...
  /**
   * Request current header chain blocks from all peers. It
   * will only run once at a time. Subsequent calls will
   * defer a call until after the current call is complete
   * unless the pool is disconnecting.
   * @private
   * @param {Peer} peer
   */
    ...
    if (!this.resolvePending) {
      this.resolvePending = (async () => {
        try {
          ...
          this.resolvePending = null;
          if (this.resolveDeferred && !this.disconnecting) {
            this.resolveDeferred = false;
            this.resolveHeaders();
          }
        }
      ...
      this.resolveDeferred = true;
    }
  }

  /**
   * Request current header chain blocks from all peers.
   * @private
   * @param {Peer} peer
   * @returns {Promise}
   */
  ...
  async _resolveHeaders() {
    // Wait for recent headers before downloading blocks.
    if (!this.options.isRecent())
      return;

    // Wait for more in-flight blocks to download.
    const max = common.BLOCK_DOWNLOAD_WINDOW - common.MAX_BLOCKS_PER_PEER;
    if (this.blockMap.size > max)
      return;

    for (let peer = this.peers.head(); peer; peer = peer.next) {
      if (peer.outbound)
        await this.resolveHeadersForPeer(peer);
    }
  }

  /**
   * Request current header chain blocks from a peer.
   * @private
   * @param {Peer} peer
   * @returns {Promise}
   */

  async resolveHeadersForPeer(peer) {
    if (!this.isSyncable(peer))
      return;

    // Don't request any more blocks if the peer has
    // pending blocks in-flight.
    if (peer.blockMap.size > 0)
      return;

    // We don't know where this peer is in the chain.
    if (!peer.bestHash) {
      // Ask for latest headers to discover the
      // best hash for the peer and start syncing.
      this.sendSync(peer);
      return;
    }

    // Check if we have an entry for this peer's chain tip hash.
    const best = await this.chain.getEntryByHash(peer.bestHash);
    if (!best) {
      this.logger.debug(
        'Peer best hash is not found (%s).', peer.hostname());
      return;
    }

    // This peer doesn't have a "longer" blockchain than us.
    if (best.chainwork.lte(this.chain.tip.chainwork)) {
      this.logger.debug(
        'Peer best chainwork is <= tip chainwork (%s).', peer.hostname());
      return;
    }

    // Request the next blocks for the peer.
    const staller = await this.getNextBlocks(peer, best);
    if (staller !== -1) {
      const slow = this.peers.find(staller);
      if (slow) {
        // Mark the peer as stalling the block download
        // window if it has not already been set.
        if (slow.stallingSince === -1)
          slow.stallingSince = Date.now();
      }
    }
  }

  /**
   * Locate and request next blocks for a peer. It will return
   * the id number of a peer stalling the download.
   * @private
   * @param {Peer} peer
   * @param {ChainEntry} best
   * @returns {Promise} - Returns Number.
   */

  async getNextBlocks(peer, best) {
    // See Bitcoin Core commit 341735eb8f42e898cf9d4d130709471e5d01abe2
    // Initialize the last common block for the peer.
    if (!peer.commonHash) {
      peer.commonHeight = Math.min(this.chain.height, best.height);
      peer.commonHash = await this.chain.getHash(peer.commonHeight);
    }

    // Get the last common entry and update in the case of
    // a chain reorganization.
    let walker = await this.chain.getEntryByHash(peer.commonHash);
    walker = await this.chain.commonAncestor(walker, best);
    peer.commonHash = walker.hash;
    peer.commonHeight = walker.height;

    const endHeight = peer.commonHeight + common.BLOCK_DOWNLOAD_WINDOW;
    const maxHeight = Math.min(best.height, endHeight + 1);
    ...
    while (walker.height < maxHeight) {
      const entries = await this.chain.db.getNextPath(
        best, walker.height, common.MAX_BLOCKS_PER_PEER);
      ...
      for (const entry of entries) {
        ...
        if (this.blockMap.has(entry.hash)) {
          if (waiting === -1)
            waiting = this.blockMap.get(entry.hash);
          continue;
        }

        // The block is considered invalid.
        if (await this.chain.db.hasInvalid(entry.hash))
          return staller;

        // We have verified this block already (even if pruned).
        if (await this.chain.isMainChain(entry)) {
          peer.commonHash = entry.hash;
          peer.commonHeight = entry.height;
          continue;
        }

        // The block is being processed.
        if (this.isLocked(entry.hash))
          continue;

        // We have this block already.
        if (await this.chain.db.hasBlock(entry.hash)) {
          saved.push(entry);
          continue;
        }

        // We have reached the end of the window.
        if (entry.height > endHeight) {
          ...
          if (items.length === 0 && waiting !== peer.id) {
            staller = waiting;
            break;
          }
        } else {
        ...
        if (items.length >= common.MAX_BLOCKS_PER_PEER) {
          filled = true;
          break;
        }
      }
      ...
      walker = entries[entries.length - 1];
    }

    // Attempt to connect the saved blocks to the chain.
    // It's only necessary to attach the first one, as
    // any consecutive blocks will also be connected in
    // the process. Any gaps within the saved blocks will
    // be connected to the chain when those missing
    // blocks are downloaded.
    if (saved.length > 0)
      await this.attachBlock(saved[0]);
  ...
  async handleOpen(peer) {
    ...
    // We want header block announcements!
    peer.sendPreferHeaders();
   ...
   * This is no-longer necessary because block announcements are
   * made via headers, as enabled with the `sendheaders` message.
   * Only versions before Bitcoin Core at v0.12.0 do not understand
   * the `sendheaders` message. This was prior to segwit, which
   * is now a requirement for a peer after segwit activation. Thus
   * this would have only be used for Bitcoin Core peers less that
   * v0.12.0 during the initial sync before segwit was activated,
   * even if there were many peers still running this version.
   * @method
   * @private
   * @param {Peer} peer
   * @param {Hash[]} hashes
  ...
  async handleHeaders(peer, packet) {
    ...
    if (peer.headersMax(headers.length)) {
      peer.increaseBan(100);
      return;
    }
    ...
    const unlock = await this.locker.lock();
    try {
      await this._handleHeaders(peer, packet);
    } finally {
      unlock();
    }
  }
  ...
  async _handleHeaders(peer, packet) {
    ...
    for (const header of headers) {
      if (last && !header.prevBlock.equals(last.hash())) {
        this.logger.warning(
    ...
    // Check for unconnected headers in what appears as a
    // block announcement.
    if (headers.length < common.MAX_HEADERS_REQUEST) {
      const connected = await this.chain.hasHeader(headers[0].prevBlock);
      if (!connected) {
        if (peer.headersUnconnectedMax()) {
          peer.increaseBan(20);
          return;
        }

        const best = this.chain.mostWork();
        const locator = await this.chain.getLocator(best.hash);
        peer.sendGetHeaders(locator);
        return;
      }
    }
    ...
    for (const header of headers) {
      ...
      // Prevent sending these headers right back to the same peer.
      peer.invFilter.add(hash);

      if (await this.chain.hasHeader(hash)) {
        this.logger.spam(
          'Already have headers: hash=%h (%s).',
          hash,
          peer.hostname()
        );
      ...
      try {
        entry = await this.chain.addHeader(header, flags);
      } catch (err) {
        if (err.type === 'VerifyError') {
          peer.reject('headers', err);
    ...
    // Peer has sent us the maximum amount of headers possible.
    // We can assume they have more for us, if we request it,
    // using the last entry as the locator.
    if (headers.length === common.MAX_HEADERS_REQUEST) {
      const entry = await this.chain.getEntryByHash(last.hash());

      // If the last header has less proof-of-work than the
      // current fully validated chain tip, the next request
      // will be more skeptical and less eager.
      const delay = this.calculateDelay(entry);
      if (delay > 0)
        await common.delay(delay);

      const locator = await this.chain.getLocator(entry.hash);
      peer.sendGetHeaders(locator);
    }
  ...
  /**
   * Calculate the delay in requesting the next set of headers
   * based on a chainwork. The deeper in the chain and less chainwork,
   * the less probable the chain is to breakeven with the honest main
   * chain (see section 11 of the Bitcoin Whitepaper). The delay is
   * calculated on an inverse exponential curve.
   * @method
   * @private
  ...
  calculateDelay(entry) {
    if (entry.chainwork.lt(this.chain.tip.chainwork)) {
      const depth = this.chain.tip.chainwork.sub(entry.chainwork);

      const scaled = new BN(Math.pow(common.MAX_HEADERS_DELAY, 2));
      scaled.imul(depth);
      scaled.idiv(this.chain.tip.chainwork);

      return Math.floor(Math.pow(scaled.toNumber(), 1/2));
    }
    return 0;
  }
  ...
  async _addBlock(peer, block, flags) {
    ...
    const hash = block.hash();
    const time = peer.blockMap.get(hash);
    ...
    // Update peer block time so that the peer
    // will not stall and timeout.
    peer.blockTime = Date.now();

    // Clear the block download window stalling
    // time once a block is received.
    peer.stallingSince = -1;

    // Ignore a merkleblock with a stale filter.
    if (this.spvFilterLast && time <= this.spvFilterLast) {
      this.logger.debug(
        'Received stale merkleblock %h (%s).',
        hash, peer.hostname());
   ...
   /**
   * Attempt to connect an existing block to the chain.
   * @private
   * @param {ChainEntry} entry
  ...
  async attachBlock(entry, prev)  {
    if (this.disconnecting)
      return;

    try {
      await this.chain.attach(entry);
    } catch (err) {
      if (err.type === 'VerifyError') {
        this.logger.warning(err);
        return;
  ...
  handleBad(msg, err, id) {
    const peer = this.peers.find(id);

    if (!peer) {
      this.logger.warning(
        'Could not find offending peer for bad %s: %h (%d).',
        msg, err.hash, id);
      return;
    }

    this.logger.debug(
      'Punishing peer for sending a bad %s (%s).',
      msg, peer.hostname());
  ...
  async handleCmpctBlock(peer, packet) {
    ...
    if (!peer.blockMap.has(hash)) {
      ...
      this.blockMap.set(hash, peer.id);
    }
  ...
  queueFilterLoad(noDelay = false) {
    if (!this.options.spv)
      return;

    this.spvFilterLast = Date.now();
  ...
  getBlock(peer, hashes) {
    ...
    // Update peer block time to know if a peer
    // is timing out with a request.
    peer.blockTime = Date.now();
  }
  ...
  /**
   * Test if a hash for a block is locked and
  ...
  /**
   * Test whether the chain has or has seen a block
   * or block header.
   * @method
   * @param {Hash} hash
   * @returns {Promise} - Returns Boolean.
  ...
  async hasSeenHeader(hash) {
    ...
    // Check the chain.
    if (await this.chain.hasSeenHeader(hash))
      return true;
