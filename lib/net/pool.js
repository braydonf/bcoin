...
class Pool extends EventEmitter {
  ...
  async handleBlockInv(peer, hashes) {
    ...
    peer.bestHash = hashes[hashes.length - 1];
    ...
    for (const hash of hashes) {
      if (await this.chain.isMainHash(hash)) {
        // If we already have this hash, we can update the peer's best height.
      ...
      if (!await this.hasBlock(hash)) {
        // See: Bitcoin Core commit 037159cebf1eae4445050cec029986514ed4e9e2
        // We don't request blocks anymore until we've verified the headers.
        const best = await this.chain.getMostWorkEntry();
        await this.getHeaders(peer, best.hash, hash);
      }
    }
  }
  ...
  async _handleHeaders(peer, packet) {
    ...
    if (headers.length === common.MAX_HEADERS_REQUEST)
      await this.getHeaders(peer, last.hash());
