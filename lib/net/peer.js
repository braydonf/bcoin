...
class Peer extends EventEmitter {
  ...
  constructor(options) {
    ...
    this.minPing = -1;
    this.blockTime = -1;
    this.stallingSince = -1;
    this.headersTimeout = -1;
    ...
    this.headersUnconnected = 0;
    this.headersUnconnectedLast = 0;
  ...
  /**
   * Update the unconnected header count
   * (block announcements) and check that the
   * count has not exceeded the maximums.
   * @param {Number} count
   */

  headersUnconnectedMax() {
    const now = Date.now();

    if (now - this.headersUnconnectedLast >= 60000)
      this.headersUnconnected = 0;

    this.headersUnconnected += 1;
    this.headersUnconnectedLast = now;

    if (this.headersUnconnected > common.MAX_HEADERS_UNCONNECTED)
      return true;
  ...
  /**
   * Update the unrequested header count
   * (block announcements) and check that the
   * count has not exceeded the maximums.
   * @param {Number} count
   */
  ...
  headersMax(count) {
    ...
    if (count > common.MAX_HEADERS_REQUEST)
      return true;

    if (this.headersPending) {
      this.headersPending = false;
      return false;
    }
    ...
    if (now - this.headersLast >= 60000)
      this.headersCount = 0;

    this.headersCount += count;
    this.headersLast = now;

    if (this.headersCount > common.MAX_HEADERS_RATE)
      return true;
  ...
  addTimeout(packet) {
    ...
    switch (packet.type) {
      ...
      case packetTypes.GETHEADERS:
        this.request(packetTypes.HEADERS, timeout);
        break;
  ...
  maybeTimeout() {
    ...
    // Timeout to respond to each individual request
    // (even if that request was for multiple items)
    ...
    // Timeout to download all filter-matching transactions
    // from a peer that sent us a merkle block.
    if (this.merkleBlock) {
    ...
    // Check for headers sync timeout or clear the timeout
    // if there are recent headers.
    if (!this.options.isRecent()) {
      if (this.headersTimeout > 0) {
        if (now > this.headersTimeout) {
          this.error('Peer is stalling (headers sync).');
          this.destroy();
          return;
        }
      }
    ...
    // During IBD, timeout for block downloads if the
    // blocks are pending and haven't been received.
    ...
    // Detect if a peer is stalling the download window so
    // that it can not move.
    if (this.stallingSince > 0) {
      if (now > this.stallingSince + Peer.STALL_TIMEOUT) {
        this.error('Peer is stalling block download window.');
        this.destroy();
        return;
      }
    }

    // Once IBD is done we only timeout if we specifically
    // requested something.
    if (this.options.isFull() || !this.syncing) {
    ...
    // No matter what state we're in, we should hear from every peer
    // at least once a minute, at least a ping/pong.
  ...
  /**
   * Send a `sendheaders` (BIP130) packet to a peer to
   * specify that block announcements are preferred
   * to be sent via `headers` messages.
   * @param {Number} mode
   */
  ...
  sendPreferHeaders() {
    if (this.version >= common.SENDHEADERS_VERSION) {
      this.logger.info(
        'Initializing headers-first sync (%s).',
        this.hostname());
...
/**
 * Required time for a peer to respond
 * with a block if the peer is currently
 * stalling the block download window.
 * @const {Number}
 * @default
 */

Peer.STALL_TIMEOUT = 2000;
...
/**
 * Base timeout for headers download.
 * @const {Number}
 * @default
 */

Peer.HEADERS_TIMEOUT_BASE = 15 * 60 * 1000; // 15 minutes

/**
 * Time estimated per header.
 * @const {Number}
 * @default
 */

Peer.HEADERS_TIMEOUT_PER_HEADER = 1; // 1 millisecond
...
class PeerOptions {
  ...
  constructor(options) {
    ...
    this.isRecent = PeerOptions.isRecent;
  ...
  fromOptions(options) {
    ...
    if (options.isRecent != null) {
      assert(typeof options.isRecent === 'function');
  ...
  /**
   * The chain is not available from pool and
   * thus it can not be recent.
   * @returns {Boolean}
   */
