...
class RPC extends RPCBase {
  ...
  async resendWalletTransactions(args, help) {
    ...
    const wallet = this.wallet;
    const txids = await wallet.resend();
  ...
  async listSinceBlock(args, help) {
    ...
    // This could consume a lot of memory and CPU and
    // be very slow. If this is a common information to query,
    // it would be better to be able to index by block height
