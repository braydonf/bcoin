...
class TXDB {
  ...
  async addTXMonotonicIndex(b, options) {
  ...
  async removeTXCountIndex(b, accounts, hash) {
    ...
    for (const [acct,] of accounts) {
      ...
      b.del(layout.x.encode(acct, acctCount));
      b.del(layout.w.encode(acct, hash));
    }
  }
  ...
  async erase(wtx, block) {
    ...
    // Remove the monotonic time index if
    // the transaction is confirmed.
    if (block) {
      const monotonicTime = await this.wdb.getMonotonicTime(block.hash);
      b.del(layout.g.encode(monotonicTime, hash));
    }
    ...
    for (const [acct, delta] of state.accounts) {
      ...
      // Remove monotonic indexing for the account if confirmed.
      if (block) {
        const monotonicTime = await this.wdb.getMonotonicTime(block.hash);
        b.del(layout.G.encode(acct, monotonicTime, hash));
      }
  ...
  async disconnect(wtx, block) {
    ...
    await this.removeTXCountIndex(b, state.accounts, hash);
  ...
  async getAccountCountForTX(acct, txid) {
    assert(typeof acct === 'number');
    assert(Buffer.isBuffer(txid));
