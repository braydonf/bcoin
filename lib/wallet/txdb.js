...
class TXDB {
  ...
  async add(tx, block, txindex) {
    ...
    if (block)
      assert(Number.isInteger(txindex), 'Index is required with block.');
    ...
    if (existing) {
      ...
      // Confirm transaction.
      return this.confirm(existing, block, txindex);
    }
    ...
    return this.insert(wtx, block, txindex);
  }
  ...
  async insert(wtx, block, txindex) {
    ...
    if (block)
      assert(Number.isInteger(txindex),
             'Index is required with block.');
  ...
  async confirm(wtx, block, txindex) {
    ...
    assert(Number.isInteger(txindex), 'Index is required.');
  ...
  async addCountAndTimeIndex(b, options) {
    ...
    const time = await this.wdb.getMedianTime(options.blockhash);
  ...
  async removeCountAndTimeIndex(b, options) {
    ...
    const time = await this.wdb.getMedianTime(blockhash);
  ...
  async addCountAndTimeIndexUnconfirmed(b, accounts, hash) {
    ...
    b.put(layout.v.encode(hash), fromU32(count));

    const time = util.now();
    b.put(layout.e.encode(hash), fromU32(time));
    b.put(layout.w.encode(time, count, hash));

    for (const [acct,] of accounts) {
      const acctCount = await this.getLatestUnconfirmedTXCount(acct);
      b.put(layout.U.encode(acct, acctCount), hash);
      b.put(layout.V.encode(acct, hash), fromU32(acctCount));

      b.put(layout.W.encode(acct, time, acctCount, hash));
    }
  }
  ...
  async removeCountAndTimeIndexUnconfirmed(b, accounts, hash) {
    ...
    const time = await this.getUnconfirmedTimeForTX(hash);
    b.del(layout.w.encode(time, count, hash));
    ...
    for (const [acct,] of accounts) {
      ...
      b.del(layout.W.encode(acct, time, acctCount, hash));
    }
  }
   ...
   * queried by hash, should there be a reorg and the transaction becomes
   * pending again.
   * @private
   * @param {Batch} b
   * @param {Array} accounts
   ...
   * order from a time (inclusive) and limited at a configurable
   * max of transactions.
   * @param {Number} acct
   * @param {Object} options
   * @param {Buffer} options.time
  ...
  async _listUnconfirmed(acct, options) {
    ...
    const count = await this.getUnconfirmedCountForTX(acct, options.hash);
