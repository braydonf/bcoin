...
class TXDB {
  ...
  async restoreCountAndTimeIndexUnconfirmed(b, accounts, hash) {
    let count = await this.getUndoCountForTX(hash);
  ...
  async disconnectCountAndTimeIndexUnconfirmed(b, accounts, hash) {
    ...
    b.put(layout.x.encode(hash), count.toRaw());
    b.del(layout.y.encode(hash));
  ...
  async getLatestUnconfirmedTXCount(acct) {
    ...
    const min = layout.z.min(height);
    const max = layout.z.max(height);
    ...
    const index = keys.length > 0 ? keys[0] + 1 : 0;

    return new TXCount(height, index);
  }
