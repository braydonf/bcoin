...
class TXDB {
  ...
  async addTimeIndex(b, options) {
    const monotonicTime = await this.wdb.getMonotonicTime(options.blockhash);
  ...
  async removeTimeIndex(b, options) {
    ...
    for (const [acct, delta] of options.accounts)
  ...
  async restoreCountIndexUnconfirmed(b, accounts, hash) {
    let existing = true;
    let count = await this.getUnconfirmedCountForTX(hash);
    if (count === null) {
    ...
    for (const [acct,] of accounts) {
      ...
      let acctCount = await this.getUnconfirmedCountForTX(hash, acct);
      if (acctCount === null) {
  ...
  async removeCountIndexUnconfirmed(b, accounts, hash) {
    const count = await this.getUnconfirmedCountForTX(hash);
    ...
    for (const [acct,] of accounts) {
      const acctCount = await this.getUnconfirmedCountForTX(hash, acct);
  ...
  getPendingHashes(acct) {
    ...
    // TODO support pagination here to handle
  ...
  getUnconfirmedRangeHashes(acct, options) {
    assert(typeof acct === 'number');

    // TODO support skipping by txid to support
    // querying large sets of unconfirmed.
    ...
    if (acct && acct !== -1) {
      hashes = this.bucket.values({
        gte: layout.U.min(acct, start),
        lte: layout.U.max(acct, end),
        limit: options.limit,
        reverse: options.reverse
      });
    } else {
      hashes = this.bucket.values({
        gte: layout.u.min(start),
        lte: layout.u.max(end),
        limit: options.limit,
        reverse: options.reverse
      });
    }
  ...
  async getUnconfirmedRange(acct, options) {
    const hashes = await this.getUnconfirmedRangeHashes(acct, options);
    const txs = [];
  ...
  async _listUnconfirmed(acct, options) {
    ...
    const count = await this.getUnconfirmedCountForTX(options.txid, acct);
   ...
   * @returns {Promise} - Returns Number.
   */
  async getUnconfirmedCountForTX(hash, acct) {
    assert(Buffer.isBuffer(hash));
    let raw = null;
  ...
  async getPending(acct) {
    // TODO deprecate this in favor of using unconfirmed
  ...
  async zap(acct, age) {
    ...
    // TODO limit range query and stream results
    // to remove pending transactions
    const txs = await this.getUnconfirmedRange(acct, {
      start: 0,
      end: now - age
    });
