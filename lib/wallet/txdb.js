...
class TXDB {
  ...
  /**
   * Update tx index for monotonic time for confirmed txs. Both
   * the state of `b` and `wtx` are modified.
   * @param {Batch} b
   * @param {TXRecord} wtx
  ...
  async updateMonotonicTime(b, wtx, blockhash, accounts) {
    const {hash} = wtx;

    // Remove existing indexes with the unconfirmed time
    b.del(layout.m.encode(wtx.monotonicTime, hash));
    for (const [acct,] of accounts)
      b.del(layout.M.encode(acct, wtx.monotonicTime, hash));

    // Add new indexes with confirmation time
    wtx.monotonicTime = await this.getMonotonicTime(blockhash);
    assert(wtx.monotonicTime);

    b.put(layout.m.encode(wtx.monotonicTime, hash));
    for (const [acct,] of accounts) {
      b.put(layout.M.encode(acct, wtx.monotonicTime, hash));
    }
  }
  ...
  /**
   * Will get the monotonic time for a block or otherwise
   * get the latest monotonic time.
   * @private
   * @param {Buffer?} blockhash
   * @returns {Promise}
   */
  ...
  async getMonotonicTime(blockhash) {
    ...
    if (blockhash)
      time = await this.wdb.getMonotonicTime(blockhash);

    if (!time)
      time = this.wdb.getLatestMonotonicTime();
  ...
  async add(tx, block) {
    ...
    const time = await this.getMonotonicTime(block ? block.hash : null);
    const wtx = TXRecord.fromTX(tx, time, block);
  ...
  async insert(wtx, block) {
    ...
    b.put(layout.m.encode(wtx.monotonicTime, hash), null);
    ...
    for (const [acct, delta] of state.accounts) {
      ...
      b.put(layout.M.encode(acct, wtx.monotonicTime, hash), null);
    ...
    if (block)
      await this.addTXCountIndex(b, state.accounts, hash);
  ...
  async confirm(wtx, block) {
    ...
    // Update the unconfirmed time to confirmed time for
    // the wtx and to the database in the batch. This needs
    // to go before the wtx is saved to the database so the
    // time in the wtx (TXRecord) is updated.
    this.updateMonotonicTime(b, wtx, block.hash, state.accounts);
  ...
  async addTXCountIndex(b, accounts, hash) {
    const count = await this.getTXCount();
    b.put(layout.z.encode(count), hash);
    b.put(layout.y.encode(hash), fromU32BE(count));

    for (const [acct,] of accounts) {
      const acctCount = await this.getTXCount(acct);
      b.put(layout.x.encode(acct, acctCount), hash);
      b.put(layout.w.encode(acct, hash), fromU32BE(acctCount));
    }
  }
  ...
  /**
   * Get TX count from the database.
   * @param {Number?} acct
  ...
  async getTXCount(acct) {
    ...
    if (!acct) {
      ...
      assert(typeof acct === 'number');
      min = layout.x.min(acct);
      max = layout.x.max(acct);
      parse = (key) => {
        const [,index] = layout.x.decode(key);
        return index;
      }
    }
  ...
  async listHistoryByTime(acct, time, limit, reverse) {
    ...
    if (reverse) {
      min = layout.m.min();
      max = layout.m.max(time);
    } else {
      min = layout.m.min(time);
      max = layout.m.max();
    }
  ...
  async _listHistory(acct, txid, limit, reverse, inclusive) {
    ...
    const count = await this.getCountForTX(acct, txid);
  ...
  async getCountForTX(acct, txid) {
    assert(typeof acct === 'number');
    assert(Buffer.isBuffer(txid));

    const raw = await this.bucket.get(layout.w.encode(acct, txid));
    if (!raw)
  ...
  async zap(acct, age) {
    ...
    let now = util.now();
    const time = this.wdb.getLatestMonotonicTime();
    if (time > now)
      now = time;
    ...
    for (const wtx of txs) {
      ...
      assert(now - wtx.monotonicTime >= age);
