...
class WalletDB extends EventEmitter {
  ...
  async getMedianTime(blockhash) {
    ...
    const raw = await this.db.get(layout.m.encode(blockhash));

    if (raw) {
      time = raw.readUInt32LE(0, true);
    } else {
      time = await this.client.getMedianTime(blockhash);
      await this.db.put(layout.m.encode(blockhash), fromU32(time));
    }
