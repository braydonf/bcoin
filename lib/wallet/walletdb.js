...
class WalletDB extends EventEmitter {
  ...
  async _addBlock(entry, txs) {
    ...
    await this._addMonotonicTime(entry);
  ...
  /**
   * Get the latest monotonic time from the database.
   * @returns {Promise} - Returns Number.
   */

  async getLatestMonotonicTime() {
    ...
    return keys.length > 0 ? keys[0] + 1 : util.now();
  }
