...
class WalletDB extends EventEmitter {
  ...
  async rollback(height) {
    ...
    // Must rollback wallets before rolling back the monotonic
    // time as this information is necessary.
    const txs = await this._rollbackWallets(height);
    this.logger.info('Rolled back %d WalletDB transactions.', txs);

    const blocks = await this._rollbackMonotonic(height);
    this.logger.info('Rolled back %d WalletDB monotonic block times.', blocks);

    await this.setTip(tip);
  }

  /**
   * Rollback monotonic time to an older state.
   * @param {Number} height
   * @returns {Promise}
   */
  async _rollbackMonotonic(height) {
    const iter = this.db.iterator({
      gte: layout.h.encode(height + 1),
      lte: layout.h.max(),
      reverse: true,
      values: true
    });
    ...
    await iter.each(async (key, value) => {
      const time = await this.getMonotonicTime(value);
      b.del(layout.g.encode(time));
      b.del(layout.G.encode(value));
  ...
  /**
   * Rollback wallet transactions to an older state.
   * @param {Number} height
   * @returns {Promise}
   */
  async _rollbackWallets(height) {
    const iter = this.db.iterator({
      gte: layout.b.encode(height + 1),
      lte: layout.b.max(),
      reverse: true,
      values: true
    });
  ...
  async _addBlock(entry, txs) {
    ...
    // Correct for any time warping blocks
    await this._putMonotonicTime(entry);
    ...
    for (const tx of txs) {
      if (await this._addTX(tx, tip, total))
        total += 1;
    }
  ...
  /**
   * Calculates the monotonic timestamp for a block
   * to correct for any time warping blocks and saves it
   * to the database.
   * @private
   * @param {ChainEntry} entry
   * @returns {Promise}
  ...
  async _putMonotonicTime(entry) {
    let monotonicTime = entry.time;
    const prevTime = await this.getMonotonicTime(entry.prevBlock);

    if (!prevTime)
      this.logger.warning(
        'Unknown monotonic time for (%d).',
        entry.prevBlock.toString('hex'));

    if (prevTime && monotonicTime <= prevTime)
      monotonicTime = prevTime + 1;

    const b = this.db.batch();

    b.put(layout.g.encode(monotonicTime), entry.hash);
    b.put(layout.G.encode(entry.hash), fromU32BE(monotonicTime));

    await b.write();
  }

  /**
   * Gets the monotonic timestamp for a block
   * @param {Buffer} blockhash
   * @returns {Promise}
   */

  async getMonotonicTime(blockhash) {
    const raw = await this.db.get(layout.G.encode(blockhash));
    if (!raw)
      return undefined;
  ...
  /**
   * Matches block hashes at a given time using
   * monotonic block time.
   * @param {Number} time - The time in seconds
   * @param {Number} limit - The maximum number to return
   * @returns {Promise}
   */

  async getBlocksByTime(time, limit, enc) {
    let result = [];
    const iter = this.db.iterator({
      gte: layout.g.encode(time),
      lte: layout.g.max(),
      keys: true,
      values: true,
      limit: limit
    ...
    await iter.each((key, value) => {
      result.push({
        time: layout.g.decode(key)[0],
        block: enc === 'hex' ? value.toString('hex') : value
      });
    });
    return result;
  }
  ...
  async _removeBlock(entry) {
    ...
    // Remove monotonic time for the block
    this._removeMonotonic(entry.hash);
  ...
  /**
   * Remove monotonic time for a block.
   * @private
   * @param {ChainEntry} entry
   * @returns {Promise}
   */
  async _removeMonotonic(hash) {
    const b = this.db.batch();

    const time = await this.getMonotonicTime(hash);

    b.del(layout.g.encode(time));
    b.del(layout.G.encode(hash));
  ...
  async _addTX(tx, block, index) {
    ...
    for (const wid of wids) {
      ...
      if (await wallet.add(tx, block, index)) {
        this.logger.info(
