...
class Wallet extends EventEmitter {
  ...
  async resend() {
    let wtxs = await this.txdb.listUnconfirmed(-1, {
      limit: 100,
      reverse: false
    });
    ...
    while (wtxs.length) {
      ...
      // TODO as this isn't the entire results
      // it may be necessary to grab any dependent
      // transactions. However, the transactions
      // should be order already as it's time sorted.
      ...
      wtxs = await this.txdb.listUnconfirmedAfter(-1, {
        hash: wtxs[wtxs.length - 1].hash,
        limit: 100,
        reverse: false
      });
    }
  ...
  async add(tx, block, index) {
    const unlock = await this.writeLock.lock();
    try {
      return await this._add(tx, block, index);
    } finally {
      unlock();
    }
  }
  ...
  async _add(tx, block, index) {
    const details = await this.txdb.add(tx, block, index);
  ...
  async getHistory(acct) {
    throw new Error('Deprecated: `wallet.getHistory()`.');
  }
  ...
  async getPending(acct) {
    throw new Error('Deprecated: `wallet.getPending()`.');
  }
  ...
  async getRange(acct, options) {
    throw new Error('Deprecated: `wallet.getRange()`.');
  }

  async getLast(acct, limit) {
    throw new Error('Deprecated: `wallet.getLast()`.');
  }
