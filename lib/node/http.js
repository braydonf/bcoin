...
class HTTP extends Server {
  ...
  handleAuth(socket) {
    ...
    socket.hook('set filter', (...args) => {
      ...
      // If the filter was updated from a confirmed block,
      // the pool will need to re-download blocks from peers
      // with the updated filter (SPV) and the node will
      // need to send blocks using the updated filter to the
      // wallet again to include all transactions.
      if (height && this.node.chain.height > height)
        this.node.chain.detach(height);
